test_that("Tests if imputation for features works", {
  ...
  expect_equal(sum(imputed_x$Species != x$Species), 7)
  expect_equal(mean(abs(x$Sepal.Width - imputed_x$Sepal.Width)), 0.0788364179257221, tolerance = 1e-6)
  ...
  expect_equal(sum(imputed_x$Species != x$Species), 21)
  expect_equal(mean(abs(x$Sepal.Width - imputed_x$Sepal.Width)), 0.28430190409346, tolerance = 1e-6)
