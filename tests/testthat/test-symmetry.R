test_that("Tests if RF with symmetric splits works", {
  ...
  forest <- forestry(x,y,ntree = 1,maxDepth = 1,symmetric = TRUE,seed=32)
  ...
  expect_equal(abs(y_pred[1]-y_pred[2]),
               abs(y_pred[2]-y_pred[61]), tolerance = 1e-6)
  ...
  expect_equal(all.equal(y_pred[1:5],
                         c(0.1134921, 0.7761905, 0.7761905, 0.7761905, 0.1134921),
                         tolerance = 1e-5),
               TRUE)


  context("Test enforcing monotonicity with symmetric splits")
  forest <- forestry(x,y,ntree = 1,maxDepth = 1,
                     seed = 2379,
                     symmetric = TRUE, monotonicConstraints = c(1,1,1))
  ...
  expect_equal(all.equal(order(y_pred), c(103:150,49:102,1:48)), TRUE)


  context("Make sure monotone symmetry works with deeper trees")
  forest <- forestry(x,y,symmetric = TRUE,
                     seed = 8296,
