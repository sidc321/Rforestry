...
Rcpp::List rcpp_cppPredictInterface(
  ...
  try {
    ...
    Rcpp::NumericVector predictions = Rcpp::wrap(*testForestPrediction);
  ...
  try {
    ...
    for(size_t i=0; i!=forest_dta->size(); i++){
      ...
      Rcpp::NumericVector split_val =
              Rcpp::wrap(((*forest_dta)[i]).split_val);
  ...
  try {
    ...
    for(size_t j=0; j  < forest_dta.size(); j++) {
      ...
      for (size_t i = 0; i < forest_dta[j]->size(); i++ ) {
        ...
        Rcpp::IntegerVector leafAveidx = Rcpp::wrap((*(forest_dta[j]))[i].var_id);
        // std::cout << "leafAveidx\n";
        // std::cout.flush();

        Rcpp::IntegerVector leafSplidx = Rcpp::wrap((*(forest_dta[j]))[i].var_id);
        // std::cout << "leafSplidx\n";
        // std::cout.flush();
