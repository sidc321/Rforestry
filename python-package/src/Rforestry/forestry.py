...
class RandomForest:
    ...
    def translate_tree_python_versiontwo(self, tree_ids = None):
        ...
        for cur_id in idx:
            ...
            numSplitidx = math.floor(self.splitratio * self.sampsize)
            numAvidx = numSplitidx if (self.splitratio == 0 or self.splitratio == 0) else self.sampsize - numSplitidx
            n_arr = ctypes.c_double * (numNodes*8 + numSplitidx + numAvidx + 1)
            tree_info = n_arr()

            lib.fill_tree_info.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.POINTER(n_arr)]
            ...
            print(self.Py_forest[cur_id]['children_right'])

            # self.Py_forest[cur_id]['splitting_sample_idx'] = np.empty(numSplitidx, dtype=np.intc)
            # for i in range(numSplitidx):
            #     self.Py_forest[cur_id]['splitting_sample_idx'][i] = int(tree_info[ind])
            #     ind += 1

            # self.Py_forest[cur_id]['averaging_sample_idx'] = np.empty(numAvidx, dtype=np.intc)
            # for i in range(self.Py_forest[cur_id]['averaging_sample_idx'].size):
            #     self.Py_forest[cur_id]['averaging_sample_idx'][i] = int(tree_info[ind])
            #     ind+=1

            
            # self.Py_forest[cur_id]['seed'] = int(tree_info[ind])
