...
def test_different_predictions():
    ...
    forest1 = forestry(ntree=1, maxDepth = 2, seed=1, scale=False)
    forest1.fit(X, y)
    p1 = forest1.predict(X)

    forest2 = forestry(ntree=1, maxDepth = 2, seed=1, scale=False)
    forest2.fit(X, y)
    ...
    forest3 = forestry(ntree=1, maxDepth = 2, seed=1, scale=False)
    forest3.fit(X2, y2)
